...
parser.add_argument('--use_wandb', type=str2bool, default=False) # Use Logging tool
...
class Model:
    ...
    def _build_net(self):
        with tf.compat.v1.variable_scope(self.name, reuse = tf.compat.v1.AUTO_REUSE):
            ...
            self.labelWeight = tf.compat.v1.placeholder(tf.float32, shape=[None, num_output], name= "label_weight")
            ...
            self.cost = tf.reduce_mean(tf.math.multiply(self.labelWeight, tf.nn.weighted_cross_entropy_with_logits(labels=self.Y, logits=self.logits, pos_weight=cross_entropy_weight)))
    ...
    def get_mean_error_hypothesis(self, x_test, y_test, label_weight, drop_out_rate=0.0, is_train=False):
        return self.sess.run([self.accuracy,  self.l2_reg, self.cost, self.hypothesis], feed_dict={self.X: x_test, self.Y: y_test, self.labelWeight: label_weight, self.drop_out_rate: drop_out_rate, self.is_train: is_train})

    def train(self, x_data, y_data, label_weight, drop_out_rate=0.0, is_train=True):
        return self.sess.run([self.accuracy, self.l2_reg, self.cost, self.optimizer], feed_dict={
            self.X: x_data, self.Y: y_data, self.labelWeight: label_weight, self.drop_out_rate: drop_out_rate, self.is_train: is_train})
...
if args.use_tf_record is True:
    ...
    TrainDataRaw = pd.read_parquet('../data_tro/TrainingData4Weighted.parquet').to_numpy().astype('float32')
    TrainData = tf.data.Dataset.from_tensor_slices((TrainDataRaw[:,0:num_input], TrainDataRaw[:,-num_output-num_output:-num_output], TrainDataRaw[:,-num_output:]))
    TrainData = TrainData.shuffle(buffer_size=100*batch_size)
...
train_batch_x, train_batch_y, train_batch_weight = Trainiterator.get_next()
data_load_finish_time = time.time()
...
for epoch in range(training_epochs):
    ...
    while True:
        try:
            x,y,label_weight = sess.run([train_batch_x, train_batch_y, train_batch_weight])
            if (x.shape[0]==batch_size):
                accu, reg_c, cost,_ = m1.train(x_data=x, y_data=y, label_weight= label_weight, drop_out_rate=drop_out_rate)
    ...
    print("Training Time: ", train_finish_time - train_start_time)
    # Validation Evaluation
    accu_val, reg_val, cost_val, _ = m1.get_mean_error_hypothesis(X_validation, Y_validation, Weight_validation)
    ...
    if ((epoch % test_every) == 0 or (epoch == training_epochs - 1)):
        # Test Evaluation
        accu_test, reg_test, cost_test, hypo  = m1.get_mean_error_hypothesis(X_Test, Y_Test, Weight_Test)
        ...
        accu_test, reg_test, cost_test, hypofree  = m1.get_mean_error_hypothesis(X_TestFree, Y_TestFree, Weight_TestFree)
